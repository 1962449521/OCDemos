...
static const CGFloat kHeightOfTopScrollView = 32.0f;
...
{
    //创建顶部可滑动的tab
    _topScrollView = [[[NSBundle mainBundle]loadNibNamed:@"TopScrollView" owner:self options:nil]lastObject];
    [_topScrollView setX:DEVICE_screenWidth/2 - _topScrollView.width/2];
...
- (void)buildUI
{
    ...
    for (int i=0; i<number; i++) {
        ...
        UIView *testView = vc.view;
        [_rootScrollView addSubview:testView];
    }
...
- (void)createNameButtons
{
    ...
    //顶部tabbar的总长度
    CGFloat topScrollViewContentWidth = 314;//DEVICE_screenWidth;
    //每个tab偏移量
    CGFloat xOffset = 0;
    for (int i = 0; i < [_viewArray count]; i++) {
        ...
        [button setFrame:CGRectMake(xOffset,0,
                                    topScrollViewContentWidth / [_viewArray count], kHeightOfTopScrollView)];
        //计算下一个tab的x偏移量
        xOffset += topScrollViewContentWidth / [_viewArray count];
        
        [button setTag:i+100];
        button.selected = NO;
        if (i == 0) {
            _shadowImageView.frame = CGRectMake(0, 0, topScrollViewContentWidth / [_viewArray count], _shadowImage.size.height);
            button.selected = YES;
        }
        NSArray *titleArr = @[@"推荐", @"关注", @"附近"];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    if (scrollView == _rootScrollView) {
        _isRootScroll = YES;
        //调整顶部滑条按钮状态
        BOOL isLogin = [Coordinator isLogined];
        float contentOffsetX = scrollView.contentOffset.x;
        if(!isLogin)
        {
            if(contentOffsetX >= DEVICE_screenWidth && contentOffsetX <= 2 * DEVICE_screenWidth)
            {
                if(_isLeftScroll)
                    contentOffsetX = 2 * DEVICE_screenWidth;
                else
                    contentOffsetX = 0;
        ...
        int tag = (int)contentOffsetX/self.bounds.size.width +100;
