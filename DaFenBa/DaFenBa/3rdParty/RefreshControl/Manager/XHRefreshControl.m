...
#define kXHAutoLoadMoreRefreshedCount 40
...
- (void)endLoadMoreRefresing {
    if (self.isLoadMoreRefreshed) {
        ...
        // MARK: 成功刷新后恢复可加载页计数 如果要求页码从1开始计数，这里要-1
//        if ([self.delegate respondsToSelector:@selector(requestCurrentPage)]) {
//            self.loadMoreRefreshedCount = [self.delegate performSelector:@selector(requestCurrentPage) withObject:nil];//addbyhs
//        }
    }
}
...
//        _loadMoreView = [[XHLoadMoreView alloc] initWithFrame:CGRectMake(0, CGRectGetHeight([[UIScreen mainScreen] bounds])-kXHLoadMoreViewHeight, CGRectGetWidth([[UIScreen mainScreen] bounds]), kXHLoadMoreViewHeight)];
        // MARK: loadmoreView的位置
        _loadMoreView = [[XHLoadMoreView alloc] initWithFrame:CGRectMake(0, DEVICE_screenHeight - 20 - 44 - 44 - 49 -kXHLoadMoreViewHeight, CGRectGetWidth([[UIScreen mainScreen] bounds]), kXHLoadMoreViewHeight)];
...
- (void)setup {
    ...
    if (self.isLoadMoreRefreshed) {
        // MARK: 添加loadView到视图
        [self.scrollView.superview addSubview:self.loadMoreView];
    }
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    ...
    if ([keyPath isEqualToString:@"contentOffset"]) {
        ...
        //向上滚动超过一定距离时，要隐藏loadmoreview
        int hscurrentPostion = contentOffset.y;
        
        if (hscurrentPostion > 0) {
            
            CGRect hsbounds = self.scrollView.bounds;//边界
            
            CGSize hssize = self.scrollView.contentSize;//滚动视图内容区域size
            
            UIEdgeInsets hsinset = self.scrollView.contentInset;//视图周围额外的滚动视图区域
            
            float hsy = hscurrentPostion + hsbounds.size.height + hsinset. bottom;
            if (( hssize.height - hsy) > kXHLoadMoreViewHeight *2) {
        ...
        if (self.isLoadMoreRefreshed) {
            ...
            if (currentPostion > 0) {
                ...
                //判断是否滚动到底部
                if( currentPostion > oldContenOffset && (y - size.height) > kXHLoadMoreViewHeight && self.refreshState != XHRefreshStateLoading && self.isLoadMoreRefreshed && !self.loadMoreRefreshing && !self.noMoreDataForLoaded) {
                    //NSLog(@"slide uped");
                    [self startLoadMoreRefreshing];
                }
            }
            oldContenOffset = currentPostion;
        }
