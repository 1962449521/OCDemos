...
static char imageLoadedKey;//addedByhs
...
- (void)setImageURL:(id)imageURL
{
    ...
    if (imageURL)
    {
        [self reloadImageURLwithDefaultImage:nil];
    }
    else
    {
...
- (void)reloadImageURLwithDefaultImage:(NSString *)defaultImageName
{
    LoadComletedBlock comletedBlock;
    if (self.onLoadCompletedBlock) {
        comletedBlock = [self.onLoadCompletedBlock copy];

    }
   
    id imageURL = self.imageURL;
    if (defaultImageName == nil) {
        defaultImageName = @"remind_noimage.png";
    }
        ...
        else
        {
            if (error)
            {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    pv.progress = 0;
                    [pv removeFromSuperview];
                    UIImage *remindNoImage = [UIImage imageNamed:defaultImageName];
...
// added by hs
- (void)setOnLoadCompletedBlock:(LoadComletedBlock)onLoadCompletedBlock
{
    objc_setAssociatedObject(self, &imageLoadedKey, onLoadCompletedBlock, OBJC_ASSOCIATION_COPY_NONATOMIC);
}
- (LoadComletedBlock)onLoadCompletedBlock
{
    return objc_getAssociatedObject(self, &imageLoadedKey);
...
{
    [self sy_loadTapEvent];
    objc_setAssociatedObject(self, &imageURLKey, imageStr, OBJC_ASSOCIATION_COPY_NONATOMIC);
    if (imageStr)
    {
        [self reloadImageURLwithDefaultImage:defaultImageName];
    }
    else
    {
        [self sy_cancelCurrentImageLoad];
